...
async def predict_single(
    request: PredictionRequest, pred: ModelPredictor = Depends(get_predictor)
):
...
async def predict_batch(
    request: BatchPredictionRequest, pred: ModelPredictor = Depends(get_predictor)
):
...
async def value_error_handler(request, exc):
    return JSONResponse(
        status_code=400, content={"detail": str(exc), "type": "ValueError"}
    )
...
async def file_not_found_handler(request, exc):
    return JSONResponse(
        status_code=404, content={"detail": str(exc), "type": "FileNotFoundError"}
    )
